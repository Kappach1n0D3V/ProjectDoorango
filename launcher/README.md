...
- **Settings:** Playit preparation guide, game folder, player log, backups, and the .NET SDK download page. Select the gateway only under Overview → Join a server. Playit public-endpoint support is pending; see the [setup requirements](../README.md#playit-setup-no-router-port-forwarding).
